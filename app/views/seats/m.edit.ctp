		...
		<div class="toolbar">
		    ...
		    <!--current items for this seat--------->
				    <div id="sseat<?php echo $sn; ?>" style="margin:10px;" class="itemlist">
                                        <?php foreach ($sts[$sn]['item'] as $i) {
					    echo '<div class="itemdiv" id="itemdiv'.$sn.'k'.$i[key($i)]['id'].'-'.$i[key($i)]['mo'].'"><span class="iteml" id="'.$sn.'k'.$i[key($i)]['id'].'-'.$i[key($i)]['mo'].'"><a href="#" onclick="clicked(\''.$sn.'\',\''.$i[key($i)]['id'].'\',\''.$i[key($i)]['mo'].'\')">'.key($i).'</a><a href="#" onclick="remo(\''.$sn.'\',\''.$i[key($i)]['id'].'('.$i[key($i)]['mo'].')\',\''.$sn.'k'.$i[key($i)]['id'].'-'.$i[key($i)]['mo'].'\')">'.$html->image('close.png',array('alt'=>'Barnes POS System','style'=>'float:right;')).'</a><br/></span>';
